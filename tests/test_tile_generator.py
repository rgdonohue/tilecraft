...
class TestTileGenerator:
    ...
    def test_validate_tippecanoe_error(self, mock_run, tile_generator):
        ...
        mock_run.side_effect = subprocess.CalledProcessError(
            1, ["tippecanoe"], stderr="command failed"
        )
    ...
    def test_validate_input_files_empty_file(self, tile_generator, temp_dir):
        ...
        with pytest.raises(
            TileGenerationError, match="No valid feature files with data found"
        ):
            tile_generator._validate_and_filter_input_files(feature_files)
    ...
    def test_generate_cache_key_different_files(
        self, tile_generator, sample_geojson_files, temp_dir
    ):
        ...
        different_files["water"].write_text(
    ...
    def test_build_tippecanoe_command_basic(
        ...
        cmd = tile_generator._build_tippecanoe_command(
            sample_geojson_files, output_path, 0
        )
    ...
    def test_build_tippecanoe_command_with_bbox(
        self, tile_generator, sample_geojson_files, temp_dir
    ):
    ...
    def test_build_tippecanoe_command_retry_attempt(
        self, tile_generator, sample_geojson_files, temp_dir
    ):
        """Test tippecanoe command building for retry attempts."""
        output_path = temp_dir / "output.mbtiles"
        cmd = tile_generator._build_tippecanoe_command(
            sample_geojson_files, output_path, 1
        )
    ...
    def test_parse_tippecanoe_progress(self, tile_generator):
        ...
        assert (
            tile_generator._parse_tippecanoe_progress("Reading features from file")
            == "reading features"
        )
        assert (
            tile_generator._parse_tippecanoe_progress("Sorting features")
            == "sorting features"
        )
        assert (
            tile_generator._parse_tippecanoe_progress("Choosing a maxzoom")
            == "optimizing zoom levels"
        )
        assert (
            tile_generator._parse_tippecanoe_progress("Tile 100/1000")
            == "generating tiles"
        )
        assert (
            tile_generator._parse_tippecanoe_progress("Wrote output.mbtiles")
            == "finalizing output"
        )
        assert (
            tile_generator._parse_tippecanoe_progress("Other message") == "processing"
        )
    ...
    def test_parse_tippecanoe_error(self, tile_generator):
        ...
        assert (
            "out of memory"
            in tile_generator._parse_tippecanoe_error("out of memory error").lower()
        )
        assert (
            "killed" in tile_generator._parse_tippecanoe_error("process killed").lower()
        )
        assert (
            "input file not found"
            in tile_generator._parse_tippecanoe_error("no such file error").lower()
        )
        assert (
            "invalid geojson"
            in tile_generator._parse_tippecanoe_error("invalid geojson format").lower()
        )
        assert (
            "empty"
            in tile_generator._parse_tippecanoe_error("empty input file").lower()
        )

        # Test generic error
        generic_error = tile_generator._parse_tippecanoe_error(
            "Some other error\nSecond line"
        )
    ...
    def test_generate_with_cache_hit(
        self, mock_generate, tile_generator, sample_geojson_files, temp_dir
    ):
    ...
    @patch("tilecraft.core.tile_generator.TileGenerator._generate_tiles_internal")
    def test_generate_with_retry_on_memory_error(
        self, mock_generate, tile_generator, sample_geojson_files
    ):
    ...
    @patch("tilecraft.core.tile_generator.TileGenerator._generate_tiles_internal")
    def test_generate_with_retry_exhausted(
        self, mock_generate, tile_generator, sample_geojson_files
    ):
...
class TestTileGeneratorIntegration:
    ...
    def test_execute_tippecanoe_with_progress_success(self, mock_popen, tile_generator):
        ...
        result = tile_generator._execute_tippecanoe_with_progress(
            cmd, mock_progress, mock_task_id
        )
    ...
    def test_execute_tippecanoe_with_progress_failure(self, mock_popen, tile_generator):
        ...
        with pytest.raises(TippecanoeError, match="Tippecanoe failed"):
            tile_generator._execute_tippecanoe_with_progress(
                cmd, mock_progress, mock_task_id
            )
