...
class LayerSchema(BaseModel):
    ...
    attributes: List[FeatureAttributes] = Field(default_factory=list, description="Layer attributes")
    description: Optional[str] = Field(default=None, description="Layer description")
    
    # Tippecanoe-specific options
    simplification: Optional[float] = Field(default=None, description="Geometry simplification")
...
class TileSchema(BaseModel):
    ...
    # Metadata
    attribution: Optional[str] = Field(default="© OpenStreetMap contributors", description="Data attribution")
    bounds: Optional[List[float]] = Field(default=None, description="Tileset bounds [west, south, east, north]")
    center: Optional[List[float]] = Field(default=None, description="Tileset center [lon, lat, zoom]")
