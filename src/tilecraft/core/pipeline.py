...
class TilecraftPipeline:
    ...
    def __init__(self, config: TilecraftConfig):
        ...
        self.cache_manager = CacheManager(config.output.cache_dir, enabled=config.cache_enabled)
    ...
    def download_osm_data(self) -> Path:
        ...
        self.logger.info(f"Downloading OSM data for bbox: {self.config.bbox.to_string()}")
    ...
    def extract_features(self, osm_data_path: Path) -> Dict[str, Path]:
        ...
        self.logger.info(f"Extracting features: {[f.value for f in self.config.features.types]}")
        
        feature_files = self.feature_extractor.extract(osm_data_path, self.config.features.types)
