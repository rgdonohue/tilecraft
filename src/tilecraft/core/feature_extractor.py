...
class OSMFeatureHandler(osmium.SimpleHandler):
    ...
    def _create_way_feature(self, way) -> Optional[Dict[str, Any]]:
        ...
        is_closed = (len(coordinates) > 2 and 
    ...
    def _determine_geometry_type(self, tags, is_closed: bool) -> str:
        ...
        area_tags = {
            ...
            'place', 'tourism', 'shop', 'area'
        }
        
        # Line tags that suggest linestring geometry
        line_tags = {
            'highway', 'waterway', 'railway', 'barrier', 'power'
        }
...
class FeatureExtractor:
    ...
    def extract(self, osm_data_path: Path, feature_types: List[FeatureType]) -> Dict[str, Path]:
        """
        ...
        logger.info(f"Extracting {len(feature_types)} feature types from: {osm_data_path}")
        ...
        with Progress(
            ...
            for i, feature_type in enumerate(feature_types):
                progress.update(main_task, description=f"Extracting {feature_type.value}...")
                
                try:
                    # Check cache first
                    bbox_str = self.config.bbox.to_string()
                    cached_path = self.cache_manager.get_cached_features(feature_type.value, bbox_str)
                    
                    if cached_path and cached_path.exists():
                        feature_count = self._count_features(cached_path)
                        logger.info(f"Using cached {feature_type.value}: {cached_path} ({feature_count} features)")
                        ...
                        try:
                            ...
                            logger.info(f"Extracted {feature_count} {feature_type.value} features ({file_size:,} bytes)")
                    ...
                    raise FeatureExtractionError(f"Feature extraction failed for {feature_type.value}: {e}")
    ...
    def _extract_feature_type_with_retry(
        ...
        for attempt in range(self.MAX_RETRIES):
            try:
                ...
                last_exception = e
                logger.warning(f"Attempt {attempt + 1} failed for {feature_type.value}: {e}")
    ...
    def _extract_feature_type(
        ...
        try:
            ...
            if handler.error_count > 0:
                logger.warning(f"Encountered {handler.error_count} errors processing {feature_type.value}")
            ...
            if output_path.exists():
                try:
                    ...
                    pass
            raise OSMProcessingError(f"OSM processing failed for {feature_type.value}: {e}")
    ...
    def _get_tag_filters(self, feature_type: FeatureType) -> Dict[str, List[str]]:
        ...
        if (self.config.features.custom_tags and 
            feature_type.value in self.config.features.custom_tags):
    ...
    def get_extraction_info(self, feature_types: List[FeatureType]) -> Dict[str, Any]:
        ...
        return {
            ...
            "tag_mappings": {ft.value: self._get_tag_filters(ft) for ft in feature_types},
